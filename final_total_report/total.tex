...
当 $\alpha = \omega^2 LC = 1$，即 $\omega = 1/\sqrt{LC}$ 时，分母为
\begin{equation}
    \sin^2\frac{k_1}{2}-\sin^2\frac{k_2}{2}.
\end{equation}

作半角换元简化三角函数。令
\begin{equation}
    x=\frac{k_1}{2},\qquad y=\frac{k_2}{2},
\end{equation}
则 $\di k_1=2\di x,\ \di k_2=2\di y$，积分区域变为 $[-\pi/2,\pi/2]^2$。于是
\begin{equation}
    I(m,n;1)=4\int_{-\pi/2}^{\pi/2}\int_{-\pi/2}^{\pi/2}
    \frac{1-\cos(2mx)\cos(2ny)}
    {\sin^2 x-\sin^2 y}
    \,\di y\,\di x.
...
\begin{equation}
    ...
    \di x\,\di y = \frac{1}{2}\,\di u\,\di v.
\end{equation}

原积分区域 $x,y\in[-\pi/2,\pi/2]$ 映射为菱形区域
\begin{equation}
    \Omega'=\{(u,v): |u|+|v|\leqslant \pi\}.
\end{equation}

代入式(\ref{eq:I_half_angle})，并利用 $\di x\,\di y=\frac{1}{2}\di u\,\di v$ 以及前面的系数4：
\begin{equation}
...
\begin{align}
    &\cos(m(u+v))\cos(n(u-v)) \nonumber\\
    &=\frac{1}{2}\left[\cos((m+n)u+(m-n)v)+\cos((m-n)u+(m+n)v)\right].
    \label{eq:cos_product_expand}
\end{align}

记 $a=m+n,\ b=m-n$。式(\ref{eq:I_uv})变为
\begin{align}
    I(m,n;1)&=2\iint_{\Omega'}\frac{1}{\sin u\sin v}\,\di u\,\di v \nonumber\\
    &\quad -\iint_{\Omega'}\frac{\cos(au+bv)+\cos(bu+av)}{\sin u\sin v}\,\di u\,\di v.
    \label{eq:I_split}
\end{align}

第一项中，被积函数 $1/(\sin u\sin v)$ 关于 $u$ 和 $v$ 均为奇函数，而积分区域 $\Omega'$ 关于 $u=0$ 和 $v=0$ 对称，因此其主值积分为零：
\begin{equation}
...
\begin{align}
    I(m,n;1)
    &=-2\iint_{\Omega'}\frac{\cos(au)\cos(bv)}{\sin u\sin v}\,\di u\,\di v \nonumber\\
    &\quad +2\iint_{\Omega'}\frac{\sin(au)\sin(bv)}{\sin u\sin v}\,\di u\,\di v.
    \label{eq:I_two_terms}
\end{align}

第一项中，$\cos(au)/\sin u$ 是奇函数（分子偶、分母奇），$\cos(bv)/\sin v$ 也是奇函数，乘积为偶函数，在菱形区域上积分不一定为零。但观察发现：若将积分区域从菱形 $\Omega'$ 延拓到矩形 $[-\pi,\pi]^2$，由于被积函数在 $\Omega'$ 外的部分关于原点反对称，主值意义下可等价为矩形积分的一半。这一步骤需谨慎处理，但最终可证明（见附录B）：
\begin{equation}
...
\end{equation}
因为被积函数 $\cos(bv)/\sin v$ 是 $v$ 的奇函数，而积分区间关于原点对称。因此整个二重积分为零。
因此只剩第二项：
\begin{equation}
...
\subsection{一维积分的计算与符号确定}

对于任意整数 $K$，标准积分（主值意义下）为
\begin{equation}
