...
在临界点 $\alpha=1$，$B = -\cos k_1$，$-B = \cos k_1$。利用 Chebyshev 多项式的三角恒等式 $U_{n-1}(\cos k_1) = \dfrac{\sin(nk_1)}{\sin k_1}$（见附录\ref{C}），式 (\ref{eq:I_alpha_less_1}) 变为
\begin{equation}
    I(m,n;1) = -4\pi \int_{-\pi}^{\pi} \cos(mk_1) \frac{\sin(nk_1)}{\sin k_1} \di k_1.
    \label{eq:I_alpha_eq_1_start}
\end{equation}
利用积化和差和引理 $\int_{-\pi}^{\pi} \frac{\sin(K\theta)}{\sin\theta} \di\theta = 2\pi$（$K$ 为奇数），$0$（$K$ 为偶数）（见附录\ref{C}），可得
\begin{equation}
    \boxed{
    I(m,n;1) =
    \begin{cases}
