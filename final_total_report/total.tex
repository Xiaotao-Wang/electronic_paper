...
\begin{enumerate}
  ...
  \begin{equation}
    \begin{dcases}
      \alpha=\dfrac{V_A^0}{1-\left(\dfrac{\lambda_+}{\lambda_-}\right)^N}\\
      \beta=-\dfrac{V_A^0}{1-\left(\dfrac{\lambda_-}{\lambda_+}\right)^N}
    \end{dcases}
  \end{equation}
...
作变量代换 $z = \e{\ii\theta}$。当 $\theta$ 从 $-\pi$ 变到 $\pi$ 时，$z$ 沿单位圆周 $C$（$|z|=1$）逆时针绕行一周。在此代换下：
\begin{equation}
    \cos\theta = \frac{z+z^{-1}}{2},\qquad \cos(n\theta) = \frac{z^n+z^{-n}}{2},\qquad \di\theta = \frac{\di z}{\ii z}.
\end{equation}

代入 $J_n(B)$：
\begin{align}
    J_n(B) &= \oint_C \frac{z^n+z^{-n}}{2}\cdot \frac{2}{2B+z+z^{-1}}\cdot \dfrac{\di z}{\ii z} \nonumber\\
    &= \dfrac{1}{\ii}\oint_C \dfrac{z^n+z^{-n}}{z^2+2Bz+1} \di z.
    \label{eq:A2}
\end{align}
...
\end{equation}
第一项在 $z=0$ 处解析（$n\geqslant 0$）。第二项在 $z=0$ 处有 $n$ 阶极点（$n\geqslant 1$）。分母 $z^2+2Bz+1$ 的零点为
\begin{equation}
...
此时 $z_{\pm}$ 均为实数，一根在单位圆内，一根在圆外。以 $B>1$ 为例：$z_+ = -B+\sqrt{B^2-1} \in (-1,0)$ 在圆内，$z_- < -1$ 在圆外。圆内极点为 $z=0$（$n\geqslant 1$）和 $z=z_+$。
...
令 $B=\cosh t$（$t>0$），则 $z_+ = -\e{-t}$，$\sqrt{B^2-1} = \sinh t$。利用恒等式 $U_{n-1}(\cosh t) = \dfrac{\sinh(nt)}{\sinh t}$（见附录\ref{C}），可化简为
\begin{equation}
...
此时 $z_{\pm} = -B \pm \ii\sqrt{1-B^2}$，满足 $|z_{\pm}|=1$，两根均在单位圆周上。积分需理解为主值。采用推迟格林函数方法，令 $B \to B+\ii\varepsilon$（$\varepsilon \to 0^+$），计算后取实部。

令 $\cos\theta_0 = -B$，$\sin\theta_0 = \sqrt{1-B^2}$。在 $\varepsilon \to 0^+$ 极限下，$z_+$ 移到圆内，$z_-$ 移到圆外。留数之和为
\begin{equation}
    \operatorname{Res}_{\text{tot}} = (-1)^{n-1}U_{n-1}(B) - \ii\dfrac{\cos(n\theta_0)}{\sin\theta_0}.
\end{equation}

推迟格林函数 $J_n^{\text{ret}}(B) = 2\pi\left[(-1)^{n-1}U_{n-1}(B) - \ii\dfrac{\cos(n\theta_0)}{\sin\theta_0}\right]$。取实部得主值：
\begin{equation}
...
\section{$U_k(\cosh t) = \sinh((k+1)t)/\sinh t$ 的证明}\label{C}

此恒等式在附录\ref{B}中用于化简 $J_n(B)$。用数学归纳法证明。

$k=0$：$U_0(\cosh t) = 1 = \dfrac{\sinh t}{\sinh t}$，成立。

$k=1$：$U_1(\cosh t) = 2\cosh t = \dfrac{\sinh(2t)}{\sinh t}$，成立。

归纳步：设对 $k-1$ 和 $k$ 成立（$k\geqslant 1$）。由 Chebyshev 递推 $U_{k+1}(x) = 2xU_k(x) - U_{k-1}(x)$：
\begin{align}
...
\section{$S_K = \int_{-\pi}^{\pi} \frac{\sin(K\theta)}{\sin\theta} \di\theta$ 的计算}\label{D}
...
由 Euler 公式和二项式定理：
\begin{align}
    \cos^j\theta &= \left(\dfrac{\e{\ii\theta}+\e{-\ii\theta}}{2}\right)^j
    = \dfrac{1}{2^j}\sum_{p=0}^j \binom{j}{p} \e{\ii(j-2p)\theta} \nonumber\\
    &= \dfrac{1}{2^j}\sum_{p=0}^j \binom{j}{p} \cos(j-2p)\theta,
    \label{eq:D1}
\end{align}
...
积分 $\int\limits_{-\pi}^{\pi}\cos(Kk_1)\di k_1$ 当 $K=0$ 时为 $2\pi$，$K\neq 0$ 时为零。
